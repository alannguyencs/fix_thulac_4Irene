...
class CBTaggingDecoder:
    ...
    def dp(self):
        ...
        self.result = {}
        self.alphas = []
        print ('dp self.result', self.result)
    ...
    def segmentTag(self, raw, graph):
        print('self.result', self.result, time.time())
        print('raw, graph', raw, graph)
        ...
        for i in range(len(raw)):
            ...
            print('self.result', i, self.result)
        self.sequence = raw
        self.len = len(raw)
        start = time.clock()
        self.putValues()
        ...
        if(len(self.labelInfo[0]) < 2):
            return 1, []
        ts = []
        #------ Alan what is self.result()----

        #------ Alan------
        for i in range(self.len):
            if(i not in self.result):
                self.result[i] = 0    #default = 0  --> not included in ts
            if((i == self.len-1) or (self.labelInfo[self.result[i]][0] == '2') or (self.labelInfo[self.result[i]][0] == '3')):
                print ('result', i, self.sequence[offset:i+1], self.result[i])
