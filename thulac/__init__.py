...
class thulac:
    ...
    def __cutWithOutMethod(self, oiraw, cut_method, text = True):
        ...
        print ('__cutWithOutMethod', oiraw, cut_method, text)
        oiraw = oiraw.split('\n')
        txt = ""
        array = []
        if(text):
            ...
            for line in oiraw:
                print ('line', line, self.__seg_only)
                if(line):
                    if(self.__seg_only):
                        array += (reduce(lambda x, y: x + [[y, '']], cut_method(line), []))
                    else:
                        print (reduce(lambda x, y: x + [[y[0], y[2]]], cut_method(line), []))
    ...
    def __cutline(self, oiraw):
        ...
        print ('before decode oiraw', oiraw)
        oiraw = decode(oiraw)
        print ('after decode oiraw', oiraw)
        vec = []
